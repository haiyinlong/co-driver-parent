...
import com.leo.ad.codriver.dws.dao.DwsRegister90DaysAccumulatePkgUsrcAdMapper;
...
public class DwsRegister90DaysAccumulatePkgUsrcAdServiceImpl implements DwsTestService {
    private final DwdUserAdRecordMapper dwdUserAdRecordMapper;
    private final DwsRegister90DaysAccumulatePkgUsrcAdMapper dwsRegister90DaysAccumulatePkgUsrcAdMapper;
    private final DwBatchMapper<DwsRegister90DaysAccumulatePkgUsrcAd,
        DwsRegister90DaysAccumulatePkgUsrcAdMapper> dwBatchMapper;
    ...
    @ShowExecuteTime(name = "DwsRegister90DaysAccumulatePkgUsrcAd")
    public void syncData(Integer dates) {
        ...
        try {
            ...
            Map<String, DwsRegister90DaysAccumulatePkgUsrcAd> todayPkgVersionAdMap = new HashMap<>();
            ...
            do {
                ...
                for (DwdUserAdRecord dwdUserAdRecord : dbList) {
                    uniqueKey = DwsRegister90DaysAccumulatePkgUsrcAd.getPkgAdUniqueKey(dwdUserAdRecord);
                    // 创建一个临时对象，用于存储数据. 包、版本
                    DwsRegister90DaysAccumulatePkgUsrcAd ad = todayPkgVersionAdMap.getOrDefault(uniqueKey,
                        DwsRegister90DaysAccumulatePkgUsrcAd.of(dwdUserAdRecord.getDates(), dwdUserAdRecord.getPkg(),
            ...
            List<DwsRegister90DaysAccumulatePkgUsrcAd> previousDayList =
                dwsRegister90DaysAccumulatePkgUsrcAdMapper.queryDbList(previousDate);
            // 转化为当天的数据，清空id;
            List<DwsRegister90DaysAccumulatePkgUsrcAd> yesterdayToTodayList =
                previousDayList.stream().map(previousPkgAd -> previousPkgAd.convertToday(dates)).toList();
            // 转集合
            List<DwsRegister90DaysAccumulatePkgUsrcAd> todayHistroyList =
                dwsRegister90DaysAccumulatePkgUsrcAdMapper.queryDbList(dates);
            List<DwsRegister90DaysAccumulatePkgUsrcAd> datesPkgAdList =
                this.mergeTodayList(todayHistroyList, todayPkgVersionAdMap);
            if (CollectionUtils.isEmpty(datesPkgAdList)) {
                // 当天没有数据也要保存记录
                dwBatchMapper.batchInsert(yesterdayToTodayList, DwsRegister90DaysAccumulatePkgUsrcAdMapper.class);
                return;
            }
            Map<String, DwsRegister90DaysAccumulatePkgUsrcAd> todayMap = yesterdayToTodayList.stream().collect(
                Collectors.toMap(DwsRegister90DaysAccumulatePkgUsrcAd::getPkgAdUniqueKey, Function.identity()));

            for (DwsRegister90DaysAccumulatePkgUsrcAd dwsRegister90DaysAccumulatePkgAd : datesPkgAdList) {
                if (todayMap.containsKey(dwsRegister90DaysAccumulatePkgAd.getPkgAdUniqueKey())) {
                    DwsRegister90DaysAccumulatePkgUsrcAd aDefault = todayMap.getOrDefault(
            ...
            List<DwsRegister90DaysAccumulatePkgUsrcAd> accumulatePkgAds = todayMap.values().stream().toList();
            dwBatchMapper.batchInsert(accumulatePkgAds, DwsRegister90DaysAccumulatePkgUsrcAdMapper.class);
            // 删除没用的数据
    ...
    private List<DwsRegister90DaysAccumulatePkgUsrcAd> mergeTodayList(
        List<DwsRegister90DaysAccumulatePkgUsrcAd> dbActiveList,
        Map<String, DwsRegister90DaysAccumulatePkgUsrcAd> pkgVerAdMap) {
        if (!CollectionUtils.isEmpty(dbActiveList)) {
            Map<String, DwsRegister90DaysAccumulatePkgUsrcAd> dbMap = dbActiveList.stream().collect(
                Collectors.toMap(DwsRegister90DaysAccumulatePkgUsrcAd::getPkgAdUniqueKey, Function.identity()));
            dbMap.forEach((key, value) -> {
                if (pkgVerAdMap.containsKey(key)) {
                    DwsRegister90DaysAccumulatePkgUsrcAd dwsDailyPkgVerAdvertising = pkgVerAdMap.get(key);
